/// at... "Doing defense is exactly analogous: just rename defense to base_defense in Fighter, add defense_bonus to Equipment and fix the compilation errors."
...
impl Object {
    ...
    pub fn equip(&mut self, log: &mut Vec<(String, Color)>) {
        if self.item.is_none() {
            log.add(format!("Can't equip {:?} because it's not an Item.", self), colors::RED);
            return;
        }
    ...
    /// Remove equipment and show a message about it
    pub fn remove(&mut self, log: &mut Vec<(String, Color)>) {
        if self.item.is_none() {
            log.add(format!("Can't remove {:?} because it's not an Item.", self), colors::RED);
            return;
        }
        if let Some(ref mut equipment) = self.equipment {
            if equipment.equipped {
                equipment.equipped = false;
                log.add(format!("Removed {} from {}.", self.name, equipment.slot), colors::LIGHT_YELLOW);
            }
        } else {
            log.add(format!("Can't remove {:?} because it's not an Equipment.", self), colors::RED);
        }
    }
...
fn ai_confused(monster_id: usize, game: &mut Game, objects: &mut [Object],
    ...
    if num_turns >= 0 { // still confused
        ...
        move_by(monster_id, rand::thread_rng().gen_range(-1, 2),
                            rand::thread_rng().gen_range(-1, 2), &game.map, objects);
...
/** Mutably borrow two *separate* elements from the given slice.
    Panics when the indices are equal or out of bonds. */
    ...
    let item_chances = &mut [
        ...
        Weighted {
            weight: from_dungeon_level(
                &[Transition {level: 4, value: 25}], level
            ),
            item: Item::Lightning,
        },
        Weighted {
            weight: from_dungeon_level(
                &[Transition {level: 6, value: 25}], level
            ),
            item: Item::Fireball,
        },
        Weighted {
            ...
            let mut item = match item_choice.ind_sample(&mut rand::thread_rng()) {
                ...
                Item::Shield => {
                    ...
                    object.equipment = Some(Equipment {
                        equipped: false, slot: Slot::LeftHand, max_hp_bonus: 0, defense_bonus: 1, power_bonus: 0,
                    });
                    object
                }
            };
...
fn pick_item_up(object_id: usize, objects: &mut Vec<Object>, game: &mut Game) {
    if game.inventory.len() >= 26 {
        game.log.add(format!("Your invenoty is full, cannot pick up {}.", objects[object_id].name), colors::RED);
    } else {
        ...
        // automatically equip, if corresponding equipment slot is unused
...
fn render_all(tcod: &mut Tcod, objects: &[Object], game: &mut Game, fov_recompute: bool) {
    if fov_recompute {
        ...
        tcod.fov.compute_fov(player.x, player.y, TORCH_RADIUS,
...
fn render_bar(panel: &mut Offscreen, x: i32, y: i32, total_width: i32,
    ...
    // rendere a bar (HP, experience, etc). First calculate the width of the bar
...
fn new_game(tcod: &mut Tcod) -> (Vec<Object>, Game) {
    ...
    player.alive = true;
    player.fighter = Some(Fighter{base_max_hp: 100, hp: 100, base_defense: 1, base_power: 2,
    ...
    dagger.equipment = Some(Equipment { equipped: true, slot: Slot::LeftHand, max_hp_bonus: 0, defense_bonus: 0, power_bonus: 2});
...
fn toggle_equipment(inventory_id: usize, _objects: &mut [Object], game: &mut Game, _tcod: &mut Tcod) -> UseResult {
    ...
    if equipment.equipped {
        game.inventory[inventory_id].remove(&mut game.log);
    } else {
...
fn cast_lightning(_inventory_id: usize, objects: &mut [Object], game: &mut Game,
    ...
    if let Some(monster_id) = monster_id {
        // zap it!
        game.log.add(format!("A lightning bolt strikes the {} with a loud thunder! The damage is {} hit points.",
...
fn target_tile(tcod: &mut Tcod, objects: &[Object], game: &mut Game, max_range: Option<f32>)
    -> Option<(i32, i32)> {
...
fn monster_death(monster: &mut Object, game: &mut Game) {
    ...
    game.log.add(format!("{} is dead! You gain {} experience points.", monster.name, monster.fighter.unwrap().xp),
                 colors::ORANGE);
...
fn handle_keys(key: Key, tcod: &mut Tcod, game: &mut Game, objects: &mut Vec<Object>) -> PlayerAction {
    ...
    match (key, player_alive) {
        ...
        (Key { printable: 'i', ..}, true) => {
            // show the inventory
            let inventory_index = inventory_menu(&mut game.inventory, "Press the key next to an item to use it, or any other to cancel.\n", &mut tcod.root);
...
Defense: {}", level, fighter.xp, level_up_xp, player.max_hp(game), player.power(game), player.defense(game));
        ...
        (Key { printable: '<', ..}, true) => {
            let player_on_stairs = objects.iter().any(|obj|
                obj.pos() == objects[PLAYER].pos() && obj.name == "stairs");
            if player_on_stairs {
