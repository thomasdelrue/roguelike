...
fn message<T: Into<String>>(messages: &mut Messages, message: T, color: Color) {
    // if buffer is full, remove the first message to make room for new one
...
fn ai_confused(monster_id: usize, map: &Map, objects: &mut [Object], messages: &mut Messages,
    ...
    if num_turns >= 0 { // still confused
        ...
        move_by(monster_id, rand::thread_rng().gen_range(-1, 2),
                            rand::thread_rng().gen_range(-1, 2), map, objects);
...
fn cast_heal(_inventory_id: usize, objects: &mut [Object], messages: &mut Messages, map: &mut Map,
...
fn cast_lightning(_inventory_id: usize, objects: &mut [Object], messages: &mut Messages, map: &mut Map,
                  tcod: &mut Tcod) -> UseResult {
...
fn cast_confuse(_inventory_id: usize, objects: &mut [Object], messages: &mut Messages, map: &mut Map,
                tcod: &mut Tcod) -> UseResult {
    ...
    if let Some(monster_id) = monster_id {
        ...
        message(messages, format!("The eyes of {} look vacant, as he starts to stumble around!",
...
fn cast_fireball(_inventory_id: usize, objects: &mut [Object], messages: &mut Messages, map: &mut Map,
                 tcod: &mut Tcod) -> UseResult {
...
fn target_tile(tcod: &mut Tcod, objects: &[Object], map: &mut Map, messages: &Messages, max_range: Option<f32>)
    -> Option<(i32, i32)> {
